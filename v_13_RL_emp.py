...
def get_reward(state,prec_state,current_average_distance):
    ...
    if queens_placed == n_queens and is_valid_solution(state):
        ...
        return 2.56/(max(current_average_distance,2.50))
...
current_average_distance=0

while not get_reward(test_state,test_state,current_average_distance)==1 and counter<10000:
    counter+=1
    counter_restart+=1
    ...
    if np.count_nonzero(test_state != -1) < n_queens:
        ...
        reward = get_reward(test_state, test_state_bef,current_average_distance)
        ...
        print(f"Average Distance of the current state: {current_average_distance:.2f}")
